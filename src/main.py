...
from PyQt5.QtWidgets import QTableWidgetItem, QMenu, QLabel, QFrame, QFileDialog
...
class MainUI(QtWidgets.QMainWindow):
    ...
    showFileManagerUiSignal = QtCore.pyqtSignal(int)
    showCmdUiSignal = QtCore.pyqtSignal(int)
    ...
    def __init__(self):
        ...
        self.lb1 = QLabel('None')
        self.lb1.setStyleSheet('border: 0; color:  green;')
        self.lb2 = QLabel('None')
        ...
        self.showFileManagerUiSignal.connect(self.showFileManagerUi)
        self.showCmdUiSignal.connect(self.showCmdUi)

    def statusBarPrint(self, string, statusId):
        if statusId == 3:
            self.lb2.setText(string)
        if statusId == 2:
            self.lb1.setText(string)
        if statusId == 1:
        ...
        try:
            ...
            if action == CommandLine:
                if self.twClient.selectedItems():
                    ...
                    self.cmdUi.append((cmdui, self.cmdId))
                    # cmdui.show()
                    # send cmd to client
                    _thread.start_new_thread(self.listenSocket.sendCmd, (clientId, self.cmdId, self.showCmdUiSignal,))
                    self.cmdId += 1
            ...
            if action == FileManager:
                if self.twClient.selectedItems():
                    ...
                    self.fileManagerUi.append((fileManagerUi, self.fileManagerId))
                    # fileManagerUi.show()
                    # send cmd to client
                    _thread.start_new_thread(self.listenSocket.sendFileManager,
                                             (clientId, self.fileManagerId, self.showFileManagerUiSignal,))
    ...
    def showFileManagerUi(self, fileManagerId):
        print(fileManagerId)
        for _fileManagerUi in self.fileManagerUi:
            if _fileManagerUi[1] == fileManagerId:
                _fileManagerUi[0].show()

    def showCmdUi(self, cmdId):
        for _cmdUi in self.cmdUi:
            if _cmdUi[1] == cmdId:
                _cmdUi[0].show()
...
class FileManagerUi(QtWidgets.QMainWindow):
    ...
    statusBarShowSignal = QtCore.pyqtSignal(str, int)
    ...
    def __init__(self, fileManagerId, listenSocket, clientComputerName):
        ...
        self.statusBarShowSignal.connect(self.statusBarShow)
    ...
    def contextMenuEvent(self, pos):
        try:
            ...
            if action == upload:
                ...
                savePath = self.Ui_FileManager.leInputPath.text()
                uploadFileName, _ = QFileDialog.getOpenFileName(self, "select item upload", "",
                                                                "All Files (*);")

                _thread.start_new_thread(self.listenSocket.uploadFile,
                                         (self.fileManagerId, uploadFileName, savePath, self.statusBarShowSignal,))
            if action == downloads:
                if self.Ui_FileManager.twList.selectedItems()[2].text() == 'File':
                    self.setDisabled(True)
                    remoteFilePath = os.path.join(self.Ui_FileManager.leInputPath.text(),
                                            self.Ui_FileManager.twList.selectedItems()[0].text())

                    savePath = QFileDialog.getSaveFileName(self, 'Save File', self.Ui_FileManager.twList.selectedItems()[0].text(), "All Files (*);")
                    _thread.start_new_thread(self.listenSocket.downFile,
                                             (self.fileManagerId, remoteFilePath, savePath, self.statusBarShowSignal,))
        except Exception as e:
            print(e)
    ...
    def closeEvent(self, event):
        print(event)
        self.listenSocket.closeFileManagerSocket(self.fileManagerId)

    def statusBarShow(self, result, enable=0):
        if enable == 1:
            self.setEnabled(True)
        self.statusBar().showMessage(result)
